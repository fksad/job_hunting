...
class SingleHeadAttention(nn.Module):
    def __init__(self, model_config: ModelConfig):
        ...
        self.reset_parameters('mask_shape', torch.tril(torch.ones([self.seq_len, self.seq_len])))  # do not calculate grad
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        mask_shape = torch.tril(torch.ones(T, T))
        weight = q @ k.transpose(-1, -2) * C**.5  # B, T, T
        weight = weight.masked_fill(self.mask_shape==0, float('-inf'))
        weight = self.dropput(weight)
        attention = F.softmax(weight, dim=-1) @ v
        return attention
